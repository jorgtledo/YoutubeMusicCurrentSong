...
func currentSong() (*songSingleton, error) {
	...
	split := strings.SplitN(ytmTitle, " - ", 3)
...
func _fetchTitle(h syscall.Handle, pid uintptr) uintptr {
	...
	*/
	...
	if _loopPid == pid {
		...
		ytmTitle = syscall.UTF16ToString(_textBuff)
		return 0
	}
...
// _fetchPid attempts to fetch the current YTM pid.
func _fetchPid() (uintptr, error) {
	...
	for _, process := range processes {
		...
		// TODO: improve filtering process.
		if process.Executable() == "chrome.exe" {
			pid = process.Pid()
			break
		}
	}
